...
# CFD export mode: derivative of the same R02 Fusion loft definitions.
# The saved Spider v2 packaging document is never altered.
def run(context):
    global app,design,root,tmp,appearances,OUT
    OUT=Path(__file__).resolve().parents[2]/'cfd'/'geometry'
    OUT.mkdir(parents=True,exist_ok=True)
    (OUT/'build.log').write_text('Spider v2 closed external-airframe CFD derivative\n')
    try:
        app=C.Application.get()
        ...
        design.designType=F.DesignTypes.DirectDesignType
        root=design.rootComponent
        tmp=F.TemporaryBRepManager.get();appearances={}
        body=fuselage()
        for sign in [-1,1]: union(body,wing(sign))
        for sign in [-1,1]:
            union(body,profile_loft([(sign*q,-940-200*q/340,-1200-50*q/340,80)
                                    for q in [0,85,170,255,340]],'y','Horizontal tail',m=0,t=.10))
        union(body,profile_loft([(40+340*t,-1000-120*t,-1230-45*t,0)
                                for t in [0,.25,.5,.75,1]],'z','Vertical tail',m=0,t=.10))
        bd=root.bRepBodies.add(body);bd.name='Spider v2 sealed external airframe - unpowered'
        root.isSketchFolderLightBulbOn=False;root.isConstructionFolderLightBulbOn=False
        exp=design.exportManager
        exp.execute(exp.createSTEPExportOptions(str(OUT/'spider_v2_cfd.step')))
        exp.execute(exp.createFusionArchiveExportOptions(str(OUT/'spider_v2_cfd.f3d')))
        (OUT/'cad_provenance.json').write_text(json.dumps({'source_document':source_name,
           'source_builder':'FWRL_Detail_R02.py','derivative':'same external loft definitions before internal cuts',
           'solid':bd.isSolid,'lumps':bd.lumps.count,'volume_cm3':bd.volume,
           'min_mm':[n*10 for n in bd.boundingBox.minPoint.asArray()],
           'max_mm':[n*10 for n in bd.boundingBox.maxPoint.asArray()],
           'simplifications':['closed access covers and gaps','no internal equipment','neutral sealed controls','no propeller or thrust','no screws pushrods or launch lug']},indent=2))
        app.activeViewport.fit()
        log('SUCCESS')
        source.activate()
    except Exception:
